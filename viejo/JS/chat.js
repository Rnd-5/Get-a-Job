	  ...
	  success: function(data) {
		...
		$.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				if ($("#chatbox_"+chatboxtitle).length <= 0) {
				createChatBox(chatboxtitle,1,item.nombre);
				}
...
$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage direct-chat-msg"><div class="direct-chat-msg"><div class="direct-chat-info clearfix"><span class="direct-chat-name pull-left">'+item.nombre+'</span></div><img class="direct-chat-img" src="../admin/images/avatar/'+item.avatar+'" alt="message user image"><div class="direct-chat-text" style="font-size:12px"">'+item.m+'</div></div></div>');
				}
			}
		});
